...
Shader "SineVFX/ForceFieldAndShieldEffects/ForceFieldBasicTriplanarLWRP"
{
	Properties
	{
		_AlphaClip("Alpha Clip Tex", 2D) = "white" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN , FRONT_FACE_TYPE ase_vface : FRONT_FACE_SEMANTIC ) : SV_Target
			{
				...
				float alclip = tex2D(_AlphaClip, uv_MaskAppearNoise).r;

				float3 BakedAlbedo = 0;
				float3 BakedEmission = 0;
				float3 Color = ( ( _RampColorTint * _FinalPower * tex2D( _Ramp, appendResult88 ) * clampResult367 * alclip ) + staticSwitch336 ).rgb;
				...
				return half4( Color, Alpha * alclip );
			}
